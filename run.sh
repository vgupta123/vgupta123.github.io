#! usr/bin/bash
python jemdoc.py -c analytics.conf index
python jemdoc.py -c analytics.conf about
python jemdoc.py -c analytics.conf pubs
python jemdoc.py -c analytics.conf intern
python jemdoc.py -c analytics.conf talks
python jemdoc.py -c analytics.conf awards
python jemdoc.py -c analytics.conf service
python jemdoc.py -c analytics.conf mentor
python jemdoc.py -c analytics.conf master
python jemdoc.py -c analytics.conf phdthesis
python jemdoc.py -c analytics.conf contact
# python jemdoc.py -c analytics.conf nlpprojs
# python jemdoc.py -c analytics.conf mlprojs
